...
import ClaimCard from "@/components/asset-ui/claim-card"; 
import { Claim, SAMPLE_CLAIMS, getClaimsForHR } from "@/lib/data";
...
export default function HRDashboard() {
  ...
  const handleReviewClaim = (data: Claim | any) => {
    if ("claim" in data) {
      console.log("Payment claim:", data);
    } else {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-gray-500 dark:text-gray-400">Kelola review dan persetujuan klaim karyawan</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{totalPending}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Menunggu review</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Total Reviewed</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{totalReviewed}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Sudah direview</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 hover:shadow-lg transition-all duration-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Approval Rate</p>
              <p className="text-3xl font-bold text-gray-900 dark:text-white">{approvalRate.toFixed(0)}%</p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Tingkat persetujuan</p>
            </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          {pendingClaims.length > 3 && (
            <button
              ...
              onClick={() => router.push("/hr-review-list")}
            >
              Lihat Semua
            </button>
          )}
        ...
        <div className="grid gap-6">
          {pendingClaims.slice(0, 3).map((claim) => (
            <ClaimCard key={claim.id} claim={claim} onAction={handleReviewClaim} actionLabel="Review" />
          ))}
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-6">
          {reviewedClaims.slice(0, 3).map((claim) => (
            <ClaimCard key={claim.id} claim={claim} onAction={handleReviewClaim} showActions={false} />
          ))}
        </div>
      </div>
    </div>
